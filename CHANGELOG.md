...
## 2025-11-12 23:22:07 UTC+8 - 修復 Flask-Compress 壓縮 Socket.IO 請求問題

### 🐛 Bug 修復

**問題描述：**
- Flask-Compress 可能壓縮了 Socket.IO polling 請求的響應
- Socket.IO 的 polling 請求不應該被壓縮，否則會導致連接失敗
- 雖然已有排除配置，但可能不夠完善

**修復內容：**
- ✅ 改進 `app/config.py` 中的 `COMPRESS_EXCLUDE` 配置，添加精確匹配
- ✅ 在 `app/__init__.py` 中添加 `before_request` 鉤子，在請求處理前排除 Socket.IO 路徑
- ✅ 改進 `after_request` 鉤子，添加緩存控制頭，確保 Socket.IO 響應不被緩存
- ✅ 使用雙重保險機制：`before_request` + `after_request` + `COMPRESS_EXCLUDE`

**改進細節：**
1. **配置層面**：`COMPRESS_EXCLUDE` 現在包含精確匹配 `/socket.io` 和包含匹配 `socket.io`
2. **請求前處理**：在 `before_request` 中設置 `request.environ['flask_compress.skip'] = True`，告訴 Flask-Compress 不要壓縮
3. **響應後處理**：在 `after_request` 中移除 `Content-Encoding` 頭，並添加緩存控制頭
4. **緩存控制**：確保 Socket.IO 響應不被瀏覽器或代理服務器緩存

**影響範圍：**
- `app/config.py` - Flask-Compress 排除配置
- `app/__init__.py` - 請求/響應處理鉤子

**技術細節：**
- Flask-Compress 會在響應處理時檢查 `request.environ['flask_compress.skip']` 標記
- 如果標記為 `True`，Flask-Compress 會跳過壓縮
- `after_request` 鉤子作為雙重保險，確保即使壓縮發生也能移除壓縮頭
...
**問題描述：**
- Socket.IO 客戶端代碼中 `error.message` 訪問可能導致運行時錯誤
- 在 `connect_error` 和 `reconnect_error` 事件中，`error` 對象可能不是標準的 Error 對象
- 當 `error` 沒有 `message` 屬性時會導致警告或錯誤

**修復內容：**
- ✅ 改進 `static/js/socketio_client.js` 和 `public/static/js/socketio_client.js` 中的錯誤處理
- ✅ 添加安全的錯誤信息提取邏輯
- ✅ 支持 Error 對象、字符串和其他類型的錯誤對象
- ✅ 當錯誤對象沒有 `message` 屬性時，自動轉換為字符串

**改進細節：**
- 在 `connect_error` 事件處理中添加錯誤類型檢查
- 在 `reconnect_error` 事件處理中添加錯誤類型檢查
- 使用三元運算符安全地提取錯誤信息
- 如果 `error` 是對象且有 `message` 屬性，使用 `error.message`
- 否則將 `error` 轉換為字符串，如果為空則使用 'Unknown error'

**影響範圍：**
- `static/js/socketio_client.js` - Socket.IO 客戶端錯誤處理
- `public/static/js/socketio_client.js` - Socket.IO 客戶端錯誤處理

---

## 2025-11-12 22:53:18 UTC+8 - 修復 Socket.IO 連接 400 Bad Request 錯誤

### 🐛 Bug 修復

**問題描述：**
- Socket.IO 客戶端連接時出現 400 Bad Request 錯誤
- 連接失敗導致即時通知功能無法正常工作
- 錯誤發生在 `handle_connect` 函數中，當 session 訪問或資料庫查詢失敗時返回 `False`，導致 Socket.IO 拒絕連接

**修復內容：**
- ✅ 改進 `app/routes/websocket.py` 中的 `handle_connect` 函數
- ✅ 添加多層錯誤處理，確保連接始終成功
- ✅ Session 訪問失敗時自動降級為訪客模式
- ✅ 資料庫查詢失敗時自動降級為訪客模式
- ✅ 房間加入失敗時仍允許連接（部分功能可能受限）
- ✅ 所有錯誤情況下都返回 `True`，確保 Socket.IO 連接不會被拒絕

**改進細節：**
1. **Session 訪問保護**：使用 try-except 包裝 session 訪問，避免 session 相關錯誤導致連接失敗
2. **資料庫查詢保護**：資料庫查詢失敗時自動降級為訪客模式，不阻止連接
3. **房間加入保護**：每個房間加入操作都有獨立的錯誤處理，單個房間失敗不影響整體連接
4. **優雅降級**：任何錯誤情況下都允許基本連接，確保 Socket.IO 功能可用

**影響範圍：**
- `app/routes/websocket.py` - Socket.IO 連接處理邏輯
